...
def build_projection_matrix(intrinsic_matrix, extrinsic_matrix):
    projection_matrix = (
        np.hstack((intrinsic_matrix, np.zeros((3, 1)))) @ extrinsic_matrix
    )
...
def perspective_projection(points_to_project, projection_matrix):
    points_to_project_h = np.vstack(
        (points_to_project, np.ones(points_to_project.shape[1]))
    )  # Convert to homogeneous
    projected_points = (
        projection_matrix @ points_to_project_h
