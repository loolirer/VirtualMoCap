...
def distort_fisheye(image_point, intrinsic_matrix, fisheye_coefficients):
    ...
    if r != 0:
        [[x_d], [y_d]] = (
            normalized_image_point
            * (theta + k1 * theta**3 + k2 * theta**5 + k3 * theta**7 + k4 * theta**9)
            / r
        )
    else:
...
def interpolate_map(map):
    ...
    estimated_values = griddata(
        known_indices, known_values, missing_indices, method="nearest"
    )
...
def build_distortion_map(
    distortion_coefficients, undistortion_map, intrinsic_matrix, resolution
):
    image_shape = resolution[::-1]

    # Make undistortion maps
    map_u, map_v = undistortion_map(
        intrinsic_matrix,
    ...
    for v in range(image_shape[0]):
        for u in range(image_shape[1]):
            ...
            if (u_d >= 0 and u_d < image_shape[1]) and (
                v_d >= 0 and v_d < image_shape[0]
            ):
                map_u_d[v_d][u_d] = u
                map_v_d[v_d][u_d] = v
