...
def proximity_order(previous_blobs, current_blobs):
    distance_matrix = np.array(
        [
            [
                np.linalg.norm(previous_blob - current_blob)
...
class Synchronizer:
    def __init__(
        ...
        self.sync_blobs = np.full(
            (self.sync_PTS.size, blob_count, 2), -1.0
        )  # Non-interpolated blobs are negative
    ...
    def add_data(self, blobs, PTS):
        ...
        if len(self.async_PTS) >= self.interpolation_window:
            ...
            for blob in range(self.blob_count):
                ...
                interpolated_blob = blob_trajectory(
                    self.sync_PTS[start : end + 1]
