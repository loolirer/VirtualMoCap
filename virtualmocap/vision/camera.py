...
class Camera:
    def __init__(
        ...
        if self.distortion_model is not None:
            self.map_u_d, self.map_v_d = build_distortion_map(
    ...
    def undistort_points(self, distorted_points):
        return self.undistortion_function(
            distorted_points.reshape(1, -1, 2).astype(np.float32),
            self.intrinsic_matrix,
            self.distortion_coefficients,
            np.array([]),
            self.intrinsic_matrix,
    ...
    def noise_image(self, image_noiseless):
        return add_noise(image_noiseless=image_noiseless, snr=self.snr)
