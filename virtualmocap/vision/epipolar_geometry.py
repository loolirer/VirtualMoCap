...
def build_essential_matrix(extrinsic_matrix_reference, extrinsic_matrix_auxiliary):
    ...
    relative_transformation = extrinsic_matrix_auxiliary @ np.linalg.inv(
        extrinsic_matrix_reference
    )
...
def build_fundamental_matrix(
    intrinsic_matrix_reference, intrinsic_matrix_auxiliary, essential_matrix
):
    fundamental_matrix = (
        np.linalg.inv(intrinsic_matrix_auxiliary).T
        @ essential_matrix
        @ np.linalg.inv(intrinsic_matrix_reference)
    )
...
def epiline_order(blobs_reference, blobs_auxiliary, fundamental_matrix, tolerance=2):
    # Computing epilines
    epilines_auxiliary = cv2.computeCorrespondEpilines(
        points=blobs_reference, whichImage=1, F=fundamental_matrix
    ).reshape(-1, 3)

    blobs_auxiliary_h = np.hstack(
        (blobs_auxiliary, np.ones(blobs_auxiliary.shape[0]).reshape(-1, 1))
    )

    # Point to line distance matrix
    distance_matrix = np.abs(
        [
            [blob_auxiliary @ epiline_auxiliary for blob_auxiliary in blobs_auxiliary_h]
            for epiline_auxiliary in epilines_auxiliary
        ]
    )

    # Indication if blob is collinear to an epiline
    collinear_blobs_matrix = (
        distance_matrix < tolerance
    )  # If distance is less than tolerance px, blob is contained
    blobs_per_line = np.count_nonzero(
        collinear_blobs_matrix, axis=1
    )  # Blobs contained in each epiline
    ambiguous = np.any(
        blobs_per_line > 1
    )  # Ambiguous if there are more than one blob in an epiline
...
def decompose_essential_matrix(
    ...
    R_t_options = [[R1, t], [R1, -t], [R2, t], [R2, -t]]
