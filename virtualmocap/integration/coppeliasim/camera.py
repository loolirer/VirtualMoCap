...
class CoppeliaSim_Camera(Camera):
    def __init__(
        ...
        intrinsic_matrix = build_intrinsic_matrix(
            fov_degrees=fov_degrees, resolution=resolution
        )
    ...
    def get_image(self, api_method):
        ...
        )  # Set second argument to 1 for grayscale, 0 for RGB

        # Convert buffer into single channel image
        image_unflipped = np.frombuffer(buffer, dtype=np.uint8).reshape(
            self.image_shape
        )
